    ...
    if (optind == 0)
        ...
        if (optmode == GETOPT_ANY)
            ...
            if (options == NULL || non_options == NULL)
                {
                (void)fprintf (stderr, "out of memory (getopt)\n");
                exit (255);
                }
...
restart:
    ...
    if (c == ':' || (q = strchr (opt_str, c)) == NULL)
        ...
        if (opterr)
            {
            if (c < ' ' || c >= 127)
                (void)fprintf (stderr, "Invalid option; character code=0x%.2x\n", c);
            else
                (void)fprintf (stderr, "Invalid option `%c%c'\n", sw_char, c);
            }
        return ('?');
        }
    if (q[1] == ':')
        ...
        if (*next_opt != 0)           /* Argument given */
            ...
            if (opterr)
                (void)fprintf (stderr, "No argument for `%c%c' option\n", sw_char, c);
            c = '?';
            }
        else
            {
